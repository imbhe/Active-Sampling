estimate_targets <- function(data, weightvar) {
  ...
  injury_risk0_logmean <- with(crashes, sum(w * log(injury_risk0)) / sum(w))
  injury_risk0_logSD <- with(crashes, sqrt(sum(w * (log(injury_risk0) - injury_risk0_logmean)^2) / sum(w)))
  ...
  return(c("mean_impact_speed0" = mean_impact_speed0, 
           ...
           "injury_risk0_logmean" = injury_risk0_logmean,
           "injury_risk0_logSD" = injury_risk0_logSD))
}