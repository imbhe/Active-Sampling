...
saveas(hf, "OvershotDist.png");



% Bivariate distribution of ("undershot", overshot) time.
% biv_pmf has columns undershot time, overshot time, and probability.
biv_pmf = zeros(length(x) * (length(x) + 1) / 2 - length(x), 3); % Initialise matrix.
ix = 1;
for i = 1:length(x);
  for j = 0:(i - 2);
    biv_pmf(ix, :) = [j / 10, x(i) - j / 10, f(i)]; % (Scaled) probability of pair (undershot, overshot). 
    disp j;
    ix = ix + 1;
  end
end
biv_pmf(:,3) = biv_pmf(:,3) / sum(biv_pmf(:,3)); % Normalise;

% Plot. 
hf = figure()
scatter(biv_pmf(:,1), biv_pmf(:,2), 10, biv_pmf(:,3));
axis square;
title('Bivariate distribution of undershot and overshot times'); 
xlabel('EOFF before tauinv = 0.2 [s]');
ylabel('EOFF after tauinv = 0.2 [s]');
xlim([0,7]); 

saveas(hf, "UnderOvershotDist.png");

xlswrite('bivariate.xlsx',biv_pmf);