...
# load simulation parameter input by param_input <- read_excel("Application/Input/parameter_input_rmse_example.xlsx")
...
pb <- progress_bar$new(format = "[:bar] :percent [Elapsed time: :elapsedfull || Estimated time remaining: :eta]",
                       total = param_input$n_repetition*(tail(sampling_input$sim_order, 1)+2),
                       clear = FALSE)      
...
ggsave(sprintf(paste("Replication of main results/","Replication of Figure 5.png",sep = "")), c1, dpi = 1000, width =160, height =70, unit = "mm")
