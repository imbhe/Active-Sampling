...
print("Running replication simulation for Figure 3.")


# Load packages -----------------------------------------------------------

source("Application/Rscript/load_required_packages.R")
pkgs <- c("BSDA", "kernlab", "magrittr", "MASS", "mboost", "mgcv", "progress", 
          "ranger", "stringr", "TeachingDemos", "tidyverse")
...
for ( i in 1:nrow(params) ) {
  ...
  char2seed(sprintf("%.2f_R2_%.2f_%s", params$bandwidth[i], params$r2[i], params$normalization[i]), set = TRUE)
...
for ( i in 1:nrow(params) ) {
  if ( !(params$model[i] %in% c("const", "pps", "cv", "ratio")) ) {
    ...
    # Test if significant improvement. 
    for ( j in 1:nn ) {
